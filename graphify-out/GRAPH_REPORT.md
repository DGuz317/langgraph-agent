...
## Corpus Check
- 64 files · ~8,575 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 384 nodes · 545 edges · 47 communities (41 shown, 6 thin omitted)
- Extraction: 80% EXTRACTED · 20% INFERRED · 0% AMBIGUOUS · INFERRED: 108 edges (avg confidence: 0.72)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e161730b`
...
1. `PlannerAgent` - 30 edges
2. `MusicAgent` - 21 edges
3. `Multi-Agent System Using LangGraph` - 14 edges
4. `InvoiceAgent` - 13 edges
5. `PlannerOutput` - 11 edges
6. `build_graph()` - 10 edges
7. `MCPToolAgent` - 10 edges
8. `MusicA2AClient` - 10 edges
9. `extract_missing_fields()` - 9 edges
10. `InvoiceA2AClient` - 9 edges
...
Cohesion: 0.11
Nodes (16): PlannerAgent, PlannedTask, PlannerOutput, RuntimeError, main(), main(), run_case(), test_llm_planner_returns_structured_tasks_for_core_cases() (+8 more)
...
- **Why does `final_response_node()` connect `Community 7` to `Community 10`?**
  _High betweenness centrality (0.126) - this node is a cross-community bridge._
- **Why does `AggregatorInput` connect `Community 10` to `Community 7`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `PlannerAgent` connect `Community 1` to `Community 6`?**
  _High betweenness centrality (0.068) - this node is a cross-community bridge._
- **Are the 13 inferred relationships involving `PlannerAgent` (e.g. with `FakeStructuredLLM` and `FakeLLM`) actually correct?**
  _`PlannerAgent` has 13 INFERRED edges - model-reasoned connections that need verification._
