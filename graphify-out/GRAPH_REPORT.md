...
## Corpus Check
- 64 files · ~8,335 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 392 nodes · 566 edges · 45 communities (39 shown, 6 thin omitted)
- Extraction: 79% EXTRACTED · 21% INFERRED · 0% AMBIGUOUS · INFERRED: 117 edges (avg confidence: 0.72)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `44392c6f`
...
1. `PlannerAgent` - 31 edges
2. `MusicAgent` - 21 edges
3. `PlannerOutput` - 15 edges
4. `Multi-Agent System Using LangGraph` - 14 edges
5. `InvoiceAgent` - 13 edges
6. `PlannedTask` - 13 edges
7. `build_graph()` - 10 edges
8. `MCPToolAgent` - 10 edges
9. `MusicA2AClient` - 10 edges
10. `extract_missing_fields()` - 9 edges
...
Cohesion: 0.08
Nodes (22): PlannerAgent, PlannedTask, PlannerOutput, main(), main(), run_case(), test_llm_planner_accepts_hitl_context_for_replanning(), test_llm_planner_returns_structured_tasks_for_core_cases() (+14 more)
...
Cohesion: 0.08
Nodes (10): A2AClientError, BaseA2AClient, InvoiceA2AClient, MusicA2AClient, BaseA2AClient, RuntimeError, create_thread_config(), get_interrupt_question() (+2 more)
...
Cohesion: 0.18
Nodes (17): InvoiceA2AClient, MusicA2AClient, route_after_invoice(), route_after_planner(), build_graph(), _build_hitl_planner_input(), _extract_instruction_number(), invoice_node() (+9 more)
...
Cohesion: 0.26
Nodes (10): ask_for_missing_info(), extract_missing_fields(), _extract_number(), interrupt_for_missing_info(), test_ask_for_missing_song_title(), test_extract_artist_from_plain_answer(), test_extract_customer_id_from_plain_number(), test_extract_customer_id_from_sentence() (+2 more)
...
- **Why does `final_response_node()` connect `Community 8` to `Community 7`?**
  _High betweenness centrality (0.106) - this node is a cross-community bridge._
- **Why does `PlannerAgent` connect `Community 0` to `Community 6`?**
  _High betweenness centrality (0.072) - this node is a cross-community bridge._
- **Are the 14 inferred relationships involving `PlannerAgent` (e.g. with `FakeStructuredLLM` and `FakeLLM`) actually correct?**
...
- **Are the 13 inferred relationships involving `PlannerOutput` (e.g. with `FakeStructuredLLM` and `FakeLLM`) actually correct?**
  _`PlannerOutput` has 13 INFERRED edges - model-reasoned connections that need verification._
