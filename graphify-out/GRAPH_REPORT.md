...
## Corpus Check
- 93 files · ~21,429 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 844 nodes · 1389 edges · 69 communities (62 shown, 7 thin omitted)
- Extraction: 80% EXTRACTED · 20% INFERRED · 0% AMBIGUOUS · INFERRED: 275 edges (avg confidence: 0.75)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f39f9f5d`
...
3. `Multi-Agent System Using LangGraph` - 26 edges
4. `PlannedTask` - 25 edges
5. `InvoiceAgent` - 24 edges
6. `PlannerOutput` - 23 edges
7. `extract_missing_fields()` - 20 edges
8. `MCPToolAgent` - 20 edges
9. `invoice_node()` - 19 edges
10. `_call_tool()` - 17 edges
...
Cohesion: 0.17
Nodes (12): Architecture Overview, code:text (User input), Configuration Notes, Core Boundaries, Current Architecture, Current Checkpoint, Current Checkpoint: Planner Reliability Hardening, File Structure (+4 more)

### Community 3 - "Community 3"
Cohesion: 0.09
Nodes (37): AgentResult, AggregatorAgent, InvoiceA2AClient, MusicA2AClient, route_after_invoice(), route_after_planner(), build_graph(), _attach_a2a_payload() (+29 more)

### Community 4 - "Community 4"
Cohesion: 0.10
Nodes (8): InvoiceAgent, InvoiceRequest, InvoiceAgentExecutor, InvoiceAgentResponse, MCPToolAgent, test_invoice_agent_does_not_treat_invoice_id_as_customer_id(), test_invoice_agent_missing_customer_id_fails_validation(), test_invoice_agent_parse_request()
...
Cohesion: 0.06
Nodes (20): A2AClientError, BaseA2AClient, InvoiceA2AClient, MusicA2AClient, BaseA2AClient, A2AClientError, Raised when an A2A service request fails., StubA2AClient (+12 more)
...
Cohesion: 0.14
Nodes (14): load_agent_card(), Registered A2A Agent Identifiers, a2a AgentExecutor, A2A DefaultRequestHandler, FastAPI Application, MusicAgent, create_app(), main() (+6 more)
...
Cohesion: 0.06
Nodes (54): 1. Start MCP server, 2. Start Invoice A2A service, 3. Start Music A2A service, 4. Run Planner CLI, 5. Or run Planner API, A2A integration test fails with connection error, Architecture, code:text (User query) (+46 more)
...
Cohesion: 0.16
Nodes (21): PlannedTask, PlannerOutput, RecordingPlanner, test_missing_info_node_preserves_album_intent_when_artist_is_supplied(), test_missing_info_node_replans_with_customer_id(), test_missing_info_node_replans_with_song_title(), test_planned_task_accepts_all_invoices_task(), test_planned_task_accepts_clarify_music_search_with_missing_search_type() (+13 more)
...
Cohesion: 0.15
Nodes (18): code:text (Get all invoices for customer_id=5), code:json ({), code:text (invoice -> music -> final_response), code:text (invoice -\), code:bash (uv run python scripts/run_mcp_server.py --host localhost --p), code:bash (uv run python scripts/run_invoice_a2a.py --host localhost --), code:bash (uv run python scripts/run_music_a2a.py --host localhost --po), code:bash (uv run python tests/test_planner_graph.py) (+10 more)
...
Cohesion: 0.14
Nodes (18): PlannerAgent, PlannerAgent.debug_raw_llm, PlannerAgent Instruction Builders, PlannerAgent Intent Detection Helpers, PlannerAgent.invoke, PlannerAgent._invoke_deterministic, PlannerAgent._invoke_llm, PlannerAgent Missing Field Detection (+10 more)
...
Cohesion: 0.22
Nodes (9): code:bash (uv run pytest tests/test_invoice_agent_parsing.py tests/test), code:bash (RUN_INVOICE_SUPPORT_INTEGRATION_TESTS=1 uv run pytest tests/), code:bash (uv run pytest tests -q), Completed Reliability Work, Current Execution Flow, Current Reliability Improvements, HITL and Aggregator, Planner Graph (+1 more)

### Community 61 - "Community 61"
Cohesion: 0.40
Nodes (5): code:text (latest_invoice -> {latest_invoice, support_employee}), code:bash (uv sync), config.py, Configuration, Current Invoice Rule

### Community 63 - "Community 63"
Cohesion: 0.22
Nodes (10): A2A Clients, A2A Services, Agent Cards, code:bash (uv run python scripts/run_mcp_server.py --host localhost --p), code:bash (uv run python scripts/run_planner.py), code:bash (uv run python scripts/run_orchestrator_api.py --host localho), code:http (POST http://localhost:12000/planner/invoke), code:json ({) (+2 more)
...
## Knowledge Gaps
- **92 isolated node(s):** `PreToolUse`, `name`, `description`, `supportedInterfaces`, `version` (+87 more)
...
- **Why does `build_graph()` connect `Community 3` to `Community 56`, `Community 17`?**
...
- **Are the 23 inferred relationships involving `PlannedTask` (e.g. with `test_normalize_output_adds_task_id_and_sets_status()` and `test_normalize_output_preserves_task_args()`) actually correct?**
  _`PlannedTask` has 23 INFERRED edges - model-reasoned connections that need verification._